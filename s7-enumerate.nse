...
local function parse_response(response, host, port, output)
  ...
  local pos, value = string.unpack("C", response, 8)
  -- unpack the second byte of the SZL-ID
  local pos, szl_id = string.unpack("C", response, 31)
  -- set the offset to 0
  local offset = 0
  ...
  if (value == 0x32) then
    ...
    pos, output["Module"] = string.unpack("z", response, 44)
    -- unpack the basic hardware information
    pos, output["Basic Hardware"] = string.unpack("z", response, 72)
    -- set version number to 0
    local version = 0
    -- parse version number
    local pos, char1, char2, char3 = string.unpack("CCC", response, 123)
...
local function second_parse_response(response, output)
  ...
  if (value == 0x32) then
    ...
    -- parse system name
    pos, output["System Name"] = string.unpack("z", response, 40 + offset)
    -- parse module type
    pos, output["Module Type"] = string.unpack("z", response, 74 + offset)
    -- parse serial number
    pos, output["Serial Number"] = string.unpack("z", response, 176 + offset)
    -- parse plant identification
    pos, output["Plant Identification"] = string.unpack("z", response, 108 + offset)
    -- parse copyright
    pos, output["Copyright"] = string.unpack("z", response, 142 + offset)
...
local COTP = string.pack("H", "0300001611e00000001400c1020100c2020" .. "102" .. "c0010a")
  -- COTP packet with a dst of 200
  local alt_COTP = string.pack("H", "0300001611e00000000500c1020100c2020" .. "200" .. "c0010a")
  -- setup the ROSCTR Packet
  local ROSCTR_Setup = string.pack("H", "0300001902f08032010000000000080000f0000001000101e0")
  -- setup the Read SZL information packet
  local Read_SZL = string.pack("H", "0300002102f080320700000000000800080001120411440100ff09000400110001")
  -- setup the first SZL request (gather the basic hardware and version number)
  local first_SZL_Request = string.pack("H", "0300002102f080320700000000000800080001120411440100ff09000400110001")
  -- setup the second SZL request
  local second_SZL_Request = string.pack("H", "0300002102f080320700000000000800080001120411440100ff090004001c0001")
  ...
  -- unpack the PDU Type
  local pos, CC_connect_confirm = string.unpack("C", response, 6)
  ...
  -- unpack the protocol ID
  local pos, protocol_id = string.unpack("C", response, 8)
